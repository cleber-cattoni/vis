...
Core.prototype._redraw = function() {
  ...
  let centerWidth = 0;
  if (this.body.origin && this.body.origin === 'flowsheet') {
  ...
  if (!this.body.reduceRedraw) { // if reduceRedraw true, then dont execute this block
    var MAX_REDRAW = 5;
    if (resized) {
  ...
  this._onResize = function() {
    ...
    if (me.dom.root) {
      ...
      if ((me.dom.root.offsetWidth != me.props.lastWidth) ||
        ...
        if (!me.body.reduceRedraw && me.body.eventOnDrawn) { // if reduceRedraw, then the event should be fired at redraw componente (ie LineGraph.redraw)
