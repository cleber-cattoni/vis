...
RangeItem.prototype.repositionX = function(limitSize, group) {
  ...
  if (dataIdItemSplit && elementHeaderWidthItem &&  elementHeaderWidth && elementHeaderWidth.offsetWidth && ['tablemode', 'tablemode_multiple_values'].indexOf(prop.type) > -1) {
    const widthElement = parseFloat(elementHeaderWidth.offsetWidth / elementHeaderWidthItem.length).toFixed(2)
    const calcPositionStart = (parseInt(dataIdItemSplit[1]) * widthElement);
    const calcPositionEnd = ((parseInt(dataIdItemSplit[1]) + 1) * widthElement);
    ...
    start = calcPositionStart;
    if (new Date(this.data.end) != new Date(this.data.start) && this.data.prop.type == 'background') {
