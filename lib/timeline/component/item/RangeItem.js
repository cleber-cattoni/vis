...
function RangeItem (data, conversion, options) {
  ...
  this.mouseoverCallback = function(event){
    me.emit('mouseover',   { data, event });
  }

  //mouseover callback method
  this.mouseoutCallback = function(event){
    me.emit('mouseout',   { data, event });
  }
...
RangeItem.prototype.redraw = function() {
  ...
  if (!dom) {
    ...
    if (this.data.ieComplexTooltip) {
      ...
      dom.frame.addEventListener("mouseenter", (event) => {
        clearTimeout(debounceTimeOutId);
        debounceTimeOutId = setTimeout(() => {
          if(event.target.querySelector('.editing-cell-component:not(.ng-hide)')){ // the cell is being edited
            return;
          }
          return this.data.mouseOver(event);
        }, 200);
      });
    }
