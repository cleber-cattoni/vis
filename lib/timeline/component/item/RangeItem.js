...
RangeItem.prototype.repositionX = function(limitSize) {
  const dataIdItemSplit = this.data.id.split('_');
  const { prop } = this.data;
  const elementHeaderWidthItem = document.querySelector('.tl-setting-bar__item');
  ...
  if (dataIdItemSplit && elementHeaderWidthItem && elementHeaderWidthItem.offsetWidth && ['tablemode', 'tablemode_multiple_values'].indexOf(prop.type) > -1) {
    const calcPositionStart = (parseInt(dataIdItemSplit[1]) * elementHeaderWidthItem.offsetWidth);
    const calcPositionEnd = ((parseInt(dataIdItemSplit[1]) + 1) * elementHeaderWidthItem.offsetWidth);
    start = calcPositionStart;
    end = (start == 0) ? elementHeaderWidthItem.offsetWidth : calcPositionEnd;
  }
