...
class TimelineChartDataAxis extends DataAxis {
  constructor(body, options, svg, linegraphOptions) {
    ...
    this.defaultOptions = {
      ...
      labelOffsetY: 2,
      iconWidth: 20,
      width: document.querySelector('.data-region.data-container-with-timeline') && parseInt(document.querySelector('.data-region.data-container-with-timeline').offsetWidth / 26) + 'px',
      visible: true,
      alignZeros: true,
      data: undefined,
      left:{
  ...
  _redrawLabels() {
    ...
    for (var key in this.groups) {
      ...
      if ((group.summary && !summaryLine) || !group.summary) {
        ...
        this.drawLabels.renderLabel(y, orientation, group, previousY);
        this.drawLines.renderLine(y, group, previousY, ((rowHeight * Object.keys(this.groups).length) + offsetY));
        summaryLine = true;
      }
    }
