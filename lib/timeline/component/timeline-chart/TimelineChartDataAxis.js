  ...
  _redrawLabels() {
    ...
    for (let key in this.groups) {
      ...
      if (group.summary && !summaryLine || !group.summary) {
        ...
        let ySummary = this.height;
        let yLabel = y;
        if (group.summary) {
          yLabel = this.height;
        } else {
          ySummary = 0;
          for (let s in this.groups) {
            const grupoSummary = this.groups[s];
        ...
        this.drawLabels.renderLabel(yLabel, orientation, group, previousY);
        this.drawLines.renderLine(y, group, previousY, ySummary);
        summaryLine = true;
      }
    }
