...
class TimelineChartLineGraph extends LineGraph {
  ...
  _convertPointsYcoordinates(datapoints, group, actualY, previousY) {
    ...
    const range = {
      max: group.summary && group.group.maxValue ? group.group.maxValue : Math.max(...listOfValues),
      min: group.summary && group.group.minValue ? group.group.minValue : Math.min(...listOfValues)
    };
  ...
  _convertAvgYcoordinates(datapoints, group, actualY, previousY) {
    ...
    const range = {
      max: group.summary && group.group.maxValue ? group.group.maxValue : Math.max(...listOfMaxValues),
      min: group.summary && group.group.minValue ? group.group.minValue : Math.min(...listOfMinValues)
    };

    for (var i = 0; i < datapoints.length; i++) {
      if (datapoints[i].referenceLine) {
        let convertedValue = 0;
        const maxValue = datapoints[i].referenceLine ? datapoints[i].y : datapoints[i].maxValue;
        const minValue = datapoints[i].referenceLine ? datapoints[i].y : datapoints[i].minValue;
