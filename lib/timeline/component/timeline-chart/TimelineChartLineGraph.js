    ...
    if (this.props.width != 0 && this.itemsData != null) {
      ...
      if (groupIds.length > 0) {
        ...
        for (i = 0; i < groupIds.length; i++) {
          ...
          if (group.summary) {
            const positionY = group.group.rowHeightId[`tl-groups_${group.id}`];
            this._convertYcoordinates(groupsData[groupIds[i]], group, (positionY * groupIds.length), 0);
          } else {
          ...
          if (groupsData[groupIds[i]].length > 0) {
            switch (group.options.style) {
              case "line":
                ...
                const line = Lines.draw(paths[groupIds[i]], group, this.framework);

                if (group.group.type === 'line') {
                  DOMutil.attachEvents(line, 'mouseenter', groupsData[groupIds[i]], (event, element, data) => callbackFunction('itemmouseenter', event, element, data));
                  DOMutil.attachEvents(line, 'mouseout', groupsData[groupIds[i]], (event, element, data) => callbackFunction('itemmouseout', event, element, data));
                  DOMutil.attachEvents(line, 'click', groupsData[groupIds[i]], (event, element, data) => callbackFunction('itemclick', event, element, data));
                }
              //explicit no break;
              case "point":
              //explicit no break;
