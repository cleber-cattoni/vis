...
class DrawLabels {
  ...
  _renderArrowAvgLabel(lineHeight, previousY, orientation, labelClass, group) {
    ...
    if (group.summary && group.group && group.group.intervalScale) {
      this._renderLineLabelWithScale({ lineHeight, orientation, labelClass, group, maxValue, minValue, avgValue });
      return; // exit
    }
  ...
  _renderLineLabel(lineHeight, previousY, orientation, labelClass, group) {
    ...
    if (group.summary && group.group && group.group.intervalScale) {
      this._renderLineLabelWithScale({ lineHeight, orientation, labelClass, group, maxValue, minValue, avgValue, referenceLine });
      return; // exit
    }
    ...
    if (maxValue === minValue || avgValue) {
      const label = avgValue !== '' ? avgValue : maxValue;
  ...
  _renderLineLabelWithScale({ lineHeight, orientation, labelClass, group, maxValue, minValue, avgValue, referenceLine }) {
    ...
    if (amountLabels && amountLabelsToFit > 0) {
      ...
      const intervalHeight = intervalScale / scaleDistance * (internHeight - amountLabels * labelHeight);
  ...
  _getGroupScaleValues(group, avgLabel = false) {
    ...
    const avgValue = groupAvgValue !== undefined ? groupAvgValue : '';
