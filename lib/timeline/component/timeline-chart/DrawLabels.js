class DrawLabels {
  ...
  _renderArrowAvgLabel(y, previousY, orientation, labelClass, group) {
    if (group.itemsData && group.itemsData.length > 0) {
      const listOfMaxValues = group.itemsData.map(item => item.referenceLine ? item.y : item.maxValue);
      const listOfMinValues = group.itemsData.map(item => item.referenceLine ? item.y : item.minValue);
