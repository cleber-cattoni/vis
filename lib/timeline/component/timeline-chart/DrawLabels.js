...
class DrawLabels {
  ...
  _renderArrowAvgLabel(lineHeight, previousY, orientation, labelClass, group) {
    ...
    let maxValue = Math.max.apply(Math, group.itemsData.map(item => item.referenceLine ? item.y : item.maxValue));
    let minValue = Math.min.apply(Math, group.itemsData.map(item => item.referenceLine ? item.y : item.minValue));
  ...
  _renderLineLabel(lineHeight, previousY, orientation, labelClass, group) {
    ...
    const maxValue = group && group.group && group.group.maxValue ? group.group.maxValue : Math.max(...values);
    const minValue = group && group.group && group.group.maxValue ? group.group.minValue : Math.min(...values);
