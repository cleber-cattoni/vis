...
class DrawLabels {
  ...
  _renderArrowAvgLabel(lineHeight, previousY, orientation, labelClass, group) {
    ...
    const { maxValue, minValue } = this._calculateMinAndMaxScale(group, true);
    ...
    const avgValue = group.itemsData[0] && group.itemsData[0].avgValue ? group.itemsData[0].avgValue : '';
  ...
  _renderLineLabel(lineHeight, previousY, orientation, labelClass, group) {
    ...
    const { maxValue, minValue } = this._calculateMinAndMaxScale(group);
  ...
  _calculateMinAndMaxScale(group, avgLabel = false) {
