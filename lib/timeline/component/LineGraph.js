...
LineGraph.prototype._convertXcoordinates = function (datapoints) {
  ...
  // TODO considerar 
  const widthLegend = parseFloat(document.querySelector('.data-region.data-container-with-timeline').offsetWidth / 26);
  let gap = 0;
  if (this.body.range.options.gap < 1) gap = (1 / this.body.range.options.gap) + this.body.range.options.gap + 1.07;
  else if (this.body.range.options.gap == 1) gap = this.body.range.options.gap + 1.4;
  else if (this.body.range.options.gap < 5) gap = (this.body.range.options.gap * 1.45);
  else gap = (this.body.range.options.gap * .74);

  for (var i = 0; i < datapoints.length; i++) {
    const dateItem = new Date(datapoints[i].x)
    const timeDiff = Math.abs(dateItem.getTime() - dateStart.getTime());
    const diffHours = parseFloat(timeDiff / (1000 * 60 * 60)).toFixed(2);
    const diffHoursEnd = (this.body.range.options.gap < 1) ? (parseFloat(diffHours) - ((this.body.range.options.gap / 2))) : parseFloat(diffHours);
    datapoints[i].screen_x = toScreen(datapoints[i].x) + this.props.width + (2 * (diffHoursEnd * gap + .5));
