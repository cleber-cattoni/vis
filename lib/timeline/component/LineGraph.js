...
LineGraph.prototype._convertXcoordinates = function (datapoints) {
  ...
  for (var i = 0; i < datapoints.length; i++) {
    if (this.body.range.options.gap == 0) datapoints[i].screen_x = this.props.width + this._calculateGapPositionVIS(datapoints[i].x, widthTimeline);
    else datapoints[i].screen_x = toScreen(datapoints[i].x) + this.props.width + this._calculateGapPositionVIS(datapoints[i].x, widthTimeline);

    datapoints[i].screen_y = datapoints[i].y; //starting point for range calculations
  }
};

LineGraph.prototype._calculateGapPositionVIS = function (x, widthTimeline) {
