...
LineGraph.prototype.setOptions = function (options) {
  if (options) {
    ...
    if (this.yAxisLeft && options.dataAxis !== undefined) {
      this.yAxisLeft.forEach(y => y.setOptions(this.options.dataAxis));
...
LineGraph.prototype._updateYAxis = function (groupIds, groupRanges) {
  ...
  if (yAxisRightUsed == true && yAxisLeftUsed == true) {
    this.yAxisLeft.forEach(y => y.drawIcons = true);
    this.yAxisRight.drawIcons = true;
  }
  else {
    this.yAxisLeft.forEach(y => y.drawIcons = false);
  ...
  if (this.yAxisRight.master == false) {
    if (yAxisRightUsed == true) {
      this.yAxisLeft.forEach(y => y.lineOffset = this.yAxisRight.width);
    }
    else {
      this.yAxisLeft.forEach(y => y.lineOffset = 0);
    }

    for (let i = 0; i < groupIds.length && this.yAxisLeft[groupIds[i]]; i++) {
      resized = this.yAxisLeft[groupIds[i]].redraw(i) || resized;
    }
