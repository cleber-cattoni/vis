...
TimeAxis.prototype._repaintLabels = function () {
  ...
  if (this.props.minorCharWidth > 0) {
    const gap = (this.options.gap > 1) ? this.options.gap : 1;
  ...
  if (this.options && this.options.widthElement) {
    const widthElement = parseFloat(this.options.widthElement);
    const gap = (this.options.gap >= 1) ? this.options.gap : (1 / this.options.gap);
    width = (gap * widthElement); 
  }
  ...
  while (step.hasNext() && count < MAX) {
    ...
    //TEST PERFORMANCE
    // width = xNext - x;
    var showMinorGrid = ((xNext - x) >= prevWidth * 0.4); // prevent displaying of the 31th of the month on a scale of 5 days
    ...
    } else if (this.options['showMinorLines']) { // minor line
      if (showMinorGrid) {
