...
DataScale.prototype.getLines = function (data) {
  ...
  if (typeof data === 'object' && data !== null && data.hasOwnProperty('values') && data.values.length) {
    var values = data.values;
    var labels = data.hasOwnProperty('labels') && data.labels.length === data.values.length ? data.labels : values;
    for (let i = 0; i < values.length; i++) {
      lines.push({major: this.is_major(values[i]), y: this.convertValue(values[i]), val: labels[i]});
    }
  } else {
