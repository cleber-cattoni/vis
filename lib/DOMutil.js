...
exports.drawPoint = function(x, y, groupTemplate, JSONcontainer, svgContainer, labelObj, props) {
  ...
  switch (groupTemplate.style) {
    ...
    case 'triangle':
      const baseWidth = 2;
      if ('UP' === groupTemplate.props.direction) {
        var polygonup = exports.getSVGElement('polygon', JSONcontainer, svgContainer);
        polygonup.setAttributeNS(null, "points", `${v.x - baseWidth},${v.y + groupTemplate.size} ${v.x + groupTemplate.size + baseWidth},${v.y + groupTemplate.size} ${v.x + groupTemplate.size * 0.5},${v.y}`);
        points.push(polygonup);
      } else {
        var polygondown = exports.getSVGElement('polygon', JSONcontainer, svgContainer);
        polygondown.setAttributeNS(null, "points", `${v.x - baseWidth},${v.y} ${v.x + groupTemplate.size + baseWidth},${v.y} ${v.x + groupTemplate.size * 0.5},${v.y + groupTemplate.size}`);
