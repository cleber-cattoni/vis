...
exports.drawPoint = function(x, y, groupTemplate, JSONcontainer, svgContainer, labelObj, props) {
  ...
  switch (groupTemplate.style) {
    ...
    case 'triangle-up':
      ...
      polygonup.setAttributeNS(null, "points", `${v.x - trinagleBaseWidth},${v.y + groupTemplate.size} ${v.x + groupTemplate.size + trinagleBaseWidth},${v.y + groupTemplate.size} ${v.x + groupTemplate.size * 0.5},${v.y}`);
      points.push(polygonup);
      break;
    case 'triangle-down':
      var polygondown = exports.getSVGElement('polygon', JSONcontainer, svgContainer);
      polygondown.setAttributeNS(null, "points", `${v.x - trinagleBaseWidth},${v.y} ${v.x + groupTemplate.size + trinagleBaseWidth},${v.y} ${v.x + groupTemplate.size * 0.5},${v.y + groupTemplate.size}`);
    ...
    case 'arrow-avg':
      ...
      const crossSize = 6;
      let difference = 0;
      let avgY = y - (crossSize / 2);

      if (props) {
        difference = props.size / 2;
      }
      
      const minLineHeight = 0;
      const minOffset = 5;
      const lineHeight = minLineHeight + difference;
      const xOffset = 1; // This make the arrow more large.
      const yOffset = (minLineHeight * 2) + difference + minOffset;

      const tLeftPoint = `${v.x - xOffset},${avgY - yOffset}`;
      const tRightPoint = `${v.x + groupTemplate.size + xOffset},${avgY - yOffset}`;
      const tBottomPoint = `${v.x + groupTemplate.size * 0.5},${avgY - (2 + yOffset) + groupTemplate.size}`;
      const tLinePoint = `${v.x + groupTemplate.size * 0.5},${avgY - (2 + yOffset) + lineHeight + groupTemplate.size}`;

      const bLeftPoint = `${v.x - xOffset},${avgY + yOffset + groupTemplate.size}`;
      const bRightPoint = `${v.x + groupTemplate.size + xOffset},${avgY + yOffset + groupTemplate.size}`;
      const bTopPoint = `${v.x + groupTemplate.size * 0.5},${avgY + (2 + yOffset)}`;
      const bLinePoint = `${v.x + groupTemplate.size * 0.5},${avgY + (2 + yOffset) - lineHeight}`;
...
exports.attachEvents = function(element, name, event, data) {
  if (element && Array.isArray(element)) {
    element.forEach((ele, index) => {
      if (Array.isArray(ele)) {
        ele.forEach(e => e.addEventListener(name, (eve) => event(eve, e, data[index])));
      } else {
        ele.addEventListener(name, (eve) => event(eve, ele, data[index]));
      }
    });
  } else {
    element.addEventListener(name, (eve) => event(eve, element, data));
  }
}

exports.detachEvents = function(element, name, event) {
  if (element && Array.isArray(element)) {
    element.forEach(ele => {
      if (Array.isArray(ele)) {
        exports.detachEvents(ele, name, event);
      } else {
        e.removeEventListener(name, event)
      }
    });
  } else {
